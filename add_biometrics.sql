...
BEGIN
    ...
    DECLARE tmp_use_vacancy boolean;
    ...
    SELECT
        (CASE WHEN PROPERTY_VALUE = 'true' THEN TRUE ELSE FALSE END) INTO tmp_use_vacancy
    FROM SYSTEM_CONFIG 
    WHERE PROPERTY_NAME = 'BEHAVIOR.REUSE_SEGMENT_VACANCY';
    IF tmp_use_vacancy THEN
        SELECT
    ...
    INSERT INTO PERSON_BIOMETRICS (BIOMETRICS_ID,CONTAINER_ID,
        EXTERNAL_ID,
        REGISTED_TS)
    VALUES (last_bio_id,p_container_id, p_external_id, ROUND(UNIX_TIMESTAMP(CURTIME(4)) * 1000));
    ...
    IF seg_record_count  < tmp_max_seg_record_count THEN
        UPDATE SEGMENTS
    ...
    ELSE
        INSERT INTO SEGMENTS (BIO_ID_START,
            ...
            CONTAINER_ID,
            REVISION)
        VALUES (l_biometrics_id, l_biometrics_id + tmp_max_seg_record_count -1, tmp_one_templte_size + 1024, 1, -1, p_container_id, -1);
        SELECT
        ...
        INSERT INTO SEGMENT_CHANGE_LOG (SEGMENT_ID,
            ...
            P_NO,
            BIOMETRICS_ID)
        VALUES (r_seg_id, -1, 0, p_external_id, p_biometric_data,CAST(NOW() AS date), p_no, r_biometric_id);
        SET r_seg_version = -1;
