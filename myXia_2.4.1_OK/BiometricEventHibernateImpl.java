...
public class BiometricEventHibernateImpl extends AbstractHibernateDao implements BiometricEventDao {
	...
	public PageResult<BiometricEventInfo> getBiometricEventInfoList(BiometricEventCriteria biometricEventCriteria,
		...
		try {
			...
			List<BiometricEventInfo> resultList = (List<BiometricEventInfo>) criteria
					.setFirstResult(pageRequest.getFirstRowIndex()).setMaxResults(pageRequest.getMaxRecords() == -1
							? (count == null ? 100 : count.intValue()) : pageRequest.getMaxRecords())
					.list();

			PageResult<BiometricEventInfo> pageResult = new PageResult<>();
			pageResult.setTotalRecords(count == null ? resultList.size() : count.intValue());
